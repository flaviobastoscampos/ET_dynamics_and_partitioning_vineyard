...
# for plotting aesthetics we assign NA to Tec and ET in the end of Nov 2021
data_d$T_TEA[data_d$date > as.POSIXct("2021-08-31 00:00:00", 
                                      format="%Y-%m-%d %H:%M:%S", 
                                      tz="GMT")] <- NA
...
# colors
my_colors <- c(ET_EC_mmd_f= "lightblue", T_TEA= "darkblue")
my_labels <- c(expression(ET[EC]), expression("T"[EC]))
...
# plot
plot_Fig_3A_2021 <- ggplot(data_d, aes(x=date))+
  ...
  geom_line(aes(y=T_TEA, color= "T_TEA"), size=0.6)+
  ...
  # years
  geom_text(x=as.POSIXct("2021-05-20 04:00:00", tz="GMT"), 
            y=6.0, 
...
# plot
plot_Fig_3A_2022 <- ggplot(data_d, aes(x=date))+
  
  geom_area(aes(y=ET_EC_mmd_f), fill= "lightblue", color= "lightblue", alpha= 0.4)+
  
  # add fluxes
  geom_line(aes(y=ET_EC_mmd_f, color= "ET_EC_mmd_f"))+    # ET_ec 
  geom_line(aes(y=T_TEA, color= "T_TEA"), size=0.6)+      # Tec
  ...
  geom_bar(data= Plt_Precip_irrig_d, aes(x=date, y= Value, fill= Variable),
           stat="identity", alpha=1.0, position="dodge")+
  scale_fill_manual(values= my_colors_bars, labels= my_labels_bars)+
  ...
  # years
  geom_text(x=as.POSIXct("2022-05-20 04:00:00", tz="GMT"),
            y=6.0,
...
# pivot
dat_box_piv <- dat_box %>%
  pivot_longer(c(`ET_EC_mmd_f`, `T_TEA`), 
...
# plot
Fig_3_box <- ggplot(dat_box_piv, aes(x=factor(Year), y=Value, fill=factor(Variable)))+
...
# panel
Fig_3_panel <- ggarrange(plot_Fig_3A_2021, plot_Fig_3A_2022, Fig_3_box,
